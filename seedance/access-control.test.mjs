...
test('authenticated user may mutate a not-yet-persisted local draft before owner_id exists', () => {
  const ordinary = { id: 'user-a', email: 'ordinary@webank.com' };
  assert.equal(canMutateVideoOwner(ordinary, ''), true);
  assert.equal(canMutateVideoOwner(ordinary, null), true);
});

test('missing authenticated identity still fails closed', () => {
  assert.equal(isVideoSuperAdmin(null), false);
  assert.equal(canMutateVideoOwner(null, 'user-a'), false);
  assert.equal(canMutateVideoOwner(null, ''), false);
});
