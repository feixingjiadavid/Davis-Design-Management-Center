const PRODUCTION_BUILD = '20260807-jianying-project-bridge-r51';
...
async function r51BridgeHealth() {
  try { const data = await r51BridgeRequest('/health', { method:'GET' }, 1800); return Boolean(data?.ok); }
  catch { return false; }
}
...
async function r51SyncProjectToJianying(groupId = r51ResolveSyncGroupId()) {
  ...
  try {
    ...
    if (!bridgeReady) {
      ...
      r51UpdateSyncProgress(0,0,'桥接器未运行','请先安装 R51 包内的 Davis 剪映桥接器。安装一次后会随 Windows 自动启动。');
      return;
    }
    ...
    console.error('[Davis Video R51] JianYing sync failed',error);
